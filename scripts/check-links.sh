...
if [ $CHECK_TYPE = "bucket" ]; then
    bucket_url="$2"
    echo "Checking links on bucket ${bucket_url}"

    # We only link to get.pulumi.com in /docs/get-started/install/ and /docs/get-started/install/versions
    yarn run blc "${bucket_url}/docs/get-started/install/versions/" --follow \
        --exclude-internal \
        --exclude "https://www*"

    yarn run blc "${bucket_url}/docs/get-started/install/" --follow \
        --exclude-internal \
        --exclude "https://www*"
fi
